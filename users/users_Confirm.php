...
$colid_rsConfirm = "0";
if (isset($_GET['UID'])) {
  $colid_rsConfirm = (get_magic_quotes_gpc()) ? $_GET['UID'] : addslashes($_GET['UID']);
}
$confirm_rsConfirm = "-1";
if (isset($_GET['code'])) {
  $confirm_rsConfirm = (get_magic_quotes_gpc()) ? $_GET['code'] : addslashes($_GET['code']);
}
mysql_select_db($database_localhost, $localhost);
$query_rsConfirm = sprintf("SELECT users.UserID FROM users WHERE users.UserVerificationCode=%s  AND users.UserID=%s", GetSQLValueString($confirm_rsConfirm, "int"),GetSQLValueString($colid_rsConfirm, "int"));
$rsConfirm = mysql_query($query_rsConfirm, $localhost) or die(mysql_error());
...
if ("" == "") // Trigger
{
  ...
  $WA_fieldNamesStr = "UserVerificationCode|UserLavel";
  $WA_fieldValuesStr = "".WA_RandomPassword(20, false, true, true, "")  ."" . $WA_AB_Split . "1";
  ...
  $WA_where_fieldValuesStr = "".$row_rsConfirm['UserID']  ."";
...
<!-- InstanceEndEditable --></div>
  ...
  <div id="mainContent">
    <form id="form1" name="form1" method="post" action="">
      ...
      <p>Please <a href="users_LogIn.php">login</a>.</p>
    </form>
</div>
  <!-- InstanceEndEditable --></div>
  </div>
