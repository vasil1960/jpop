...
?>
<?php
$rsReminder_1 = new WA_MySQLi_RS("rsReminder_1",$localhost_i,1);
$rsReminder_1->setQuery("SELECT mnscrpts.mscrID, mnscrpts.mscrFullTitle, mnscrpts.mscrCodeU, mnscrpts.mscrReviewer_1, users.UserID, users.UserEmail AS EmailReviewer_1, users.UserFirstName, users.UserLastName,mnscrDateConfirm_1,mnscrpts.mscrDateToReviewer_1 FROM mnscrpts, users WHERE mnscrpts.mscrID = ? AND mnscrpts.mscrReviewer_1 = users.UserID ");
$rsReminder_1->bindParam("i", "".($_GET['mnscrpt_id'])  ."", "-1"); //colname
$rsReminder_1->execute();
?>
...
if ("" === "") {
  ...
  $InsertQuery->bindColumn("logContent", "s", "Send Reminder To Reviewer No 1", "WA_BLANK");
...
?>
<?php
if (isset($_POST["btnSendReminder"]) || isset($_POST["btnSendReminder_x"]))     {  //WA Universal Email
  $Email = new WA_Email("waue_mscr_Reminder_1_1");
  $Email->Redirect = "mscr_RemindeSuccess.php";
  $Email->From = "Journal POP <ivilievltu@klaro-bg.com>";
  $Email->BurstSize = "200";
  $Email->BurstTime = "1";
  $Email->WaitTime = "1";
  $Email->addTo("".$rsReminder_1->getColumnVal('EmailReviewer_1')  ."");
  $Email->addTo("ivilievltu@yahoo.com");
  $Email->addTo("v.tsigov@gmail.com");
  $Email->BodyFile = "../webassist/email/waue_mscr_Reminder_1_1_body.php";
  ...
  for ($emailGroup=0; $emailGroup<sizeof($Email->To); $emailGroup++) {
    $Email->Subject = "Journal POP reminder late review ".$row_rsReminder_1['mscrCodeU']  ."";
    ...
    <div id="mainContent">
      <div id="form1_ProgressWrapper">
        <form id="form1" name="form1" method="post" action="">
          <span class="title">Reminder to Reviewer No. 1</span>
          <hr />
          <table width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            <tr>
              <td colspan="2"><label for="txtReminder"></label>
                <textarea name="txtReminder" id="txtReminder" cols="95" rows="18"><p>Dear Dr. <?php echo $rsReminder_1->getColumnVal('UserFirstName'); ?> <?php echo $rsReminder_1->getColumnVal('UserLastName'); ?>,</p>
<p>You agreed to review manuscript <?php echo $rsReminder_1->getColumnVal('mscrCodeU'); ?> ( "<?php echo $rsReminder_1->getColumnVal('mscrFullTitle'); ?>" ) and your completed review was due by <?php echo splitDate($rsReminder_1->getColumnVal('mscrDateToReviewer_1')) ; ?>.</p>
