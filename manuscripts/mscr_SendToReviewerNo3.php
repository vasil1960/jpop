...
?>
<?php
$rsReviewer_3 = new WA_MySQLi_RS("rsReviewer_3",$localhost_i,1);
$rsReviewer_3->setQuery("SELECT users.UserID,  CONCAT(users.UserFirstName,' ', users.UserLastName,'   ','( ',users.UserEmail ,' ) ') AS FullName, users.UserLavel, status.statusID, users.UserEmail, CONCAT(users.UserFirstName,' ', users.UserLastName) AS ReviewerFullName FROM users, status WHERE status.statusID= users.UserLavel AND status.statusID>=2 AND status.statusID<=3 AND users.UserID=?");
$rsReviewer_3->bindParam("i", "".((isset($_POST["txtReviewer_3"]))?$_POST["txtReviewer_3"]:"")  ."", "-1"); //colname
$rsReviewer_3->execute();
?>
<?php
...
if ("" === "") {
  ...
  $InsertQuery->bindColumn("logContent", "s", "Send To Reviewer No 3", "WA_BLANK");
...
if (($_SERVER["REQUEST_METHOD"] === "POST") && (isset($_SERVER["HTTP_REFERER"]) && strpos(urldecode($_SERVER["HTTP_REFERER"]), urldecode($_SERVER["SERVER_NAME"].$_SERVER["PHP_SELF"])) > 0) && isset($_POST)) {
  ...
  $UpdateQuery->bindColumn("mscrDateToReviewer_3", "s", "to reviewer: ".$datumZ  ." ", "WA_BLANK");
  $UpdateQuery->bindColumn("mnscConfirmReviewer_3", "s", "".$null  ."", "WA_BLANK");
  $UpdateQuery->bindColumn("mnscrCodeConfirmYes_3", "s", "".$_SESSION['confirmYes']  ."", "WA_BLANK");
  $UpdateQuery->bindColumn("mnscrCodeConfirmNo_3", "s", "".$_SESSION['confirmNo']  ."", "WA_BLANK");
  $UpdateQuery->bindColumn("mnscrDateConfirm_3", "s", "".$null  ."", "WA_BLANK");
  $UpdateQuery->bindColumn("mscrReviewer_3", "s", "".((isset($_POST["txtReviewer_3"]))?$_POST["txtReviewer_3"]:"")  ."", "WA_BLANK");
  $UpdateQuery->bindColumn("mscrReviewerFullName_3", "s", "".$rsReviewer_3->getColumnVal('ReviewerFullName')  ."", "WA_BLANK");
...
if (isset($_POST["button"]) || isset($_POST["button_x"]))     {  //WA Universal Email
  $Email = new WA_Email("waue_mscr_SendToReviewerNo3_1");
  ...
  $Email->BurstTime = "1";
  $Email->WaitTime = "1";
  $Email->addTo("".$rsReviewer_3->getColumnVal('UserEmail')  ."");
  ...
  $Email->BodyFile = "../webassist/email/waue_mscr_SendToReviewerNo3_1_body.php";
  ...
  for ($emailGroup=0; $emailGroup<sizeof($Email->To); $emailGroup++) {
    $Email->Subject = "Manuscript ".$row_rsMnscrpt['mscrCodeU']  ." for review";
...
<title>Journal-POP - Select Reviewer No 3</title>
<!-- InstanceEndEditable -->
    ...
    <div id="mainContent">
      <div id="form1_ProgressWrapper">
        <form id="form1" name="form1" method="post" action="">
          <span class="title">Select Reviewer No 3</span>
          <hr />
          <table width="100%" border="0" cellpadding="0" cellspacing="2">
            ...
            <tr>
              ...
              <td colspan="3"><select name="txtReviewer_3" id="txtReviewer_3">
                <option value="" <?php if (!(strcmp("", $rsMnscrpt->getColumnVal('mscrReviewer_3')))) {echo "selected=\"selected\"";} ?>>Select reviewer ...</option>
                <?php
while(!$rsReviewer->atEnd()) { //dyn select
?>
                <option value="<?php echo $rsReviewer->getColumnVal('UserID')?>"<?php if (!(strcmp($rsReviewer->getColumnVal('UserID'), $rsMnscrpt->getColumnVal('mscrReviewer_3')))) {echo "selected=\"selected\"";} ?>><?php echo $rsReviewer->getColumnVal('FullName')?></option>
                <?php
  $rsReviewer->moveNext();
} //dyn select
...
?>
                  ...
                  <?php //WAFV_Conditional mscr_SendToReviewerNo3.php mscrSendToReviewerNo1_218(1:)
    }
  }
}?>
                </span></td>
              <td>&nbsp;</td>
              <td>&nbsp;</td>
              <td colspan="2"><a href="mscr_Reminder_3.php?mnscrpt_id=<?php echo $rsMnscrpt->getColumnVal('mscrID'); ?>">Reminder</a></td>
            </tr>
            <tr>
              <td>&nbsp;</td>
              <td width="8%">&nbsp;</td>
