...
$query_rsConfirmNo = sprintf("SELECT mnscrpts.mscrID, mnscrpts.mscrCodeU FROM mnscrpts WHERE mnscrpts.mscrID=%s AND mnscrpts.mnscrCodeConfirmNo_2=%s", GetSQLValueString($colname1_rsConfirmNo, "int"),GetSQLValueString($colname2_rsConfirmNo, "text"));
...
if (!($totalRows_rsConfirmNo == 0)) // Trigger
{
  ...
  $WA_fieldNamesStr = "mnscConfirmReviewer_2|mnscrCodeConfirmYes_2|mnscrCodeConfirmNo_2|mnscrDateConfirm_2|mscrReviewer_2|mscrReviewerFullName_2";
  $WA_fieldValuesStr = "Not confirmed" . $WA_AB_Split . "".WA_RandomPassword(9, true, true, true, "")  ."" . $WA_AB_Split . "".WA_RandomPassword(9, true, true, true, "")  ."" . $WA_AB_Split . "".$datumZ  ."" . $WA_AB_Split . "".$null  ."" . $WA_AB_Split . "Reviewer 2";
...
if ((($_SERVER["REQUEST_METHOD"] == "POST") && (isset($_SERVER["HTTP_REFERER"]) && strpos(urldecode($_SERVER["HTTP_REFERER"]), urldecode($_SERVER["SERVER_NAME"].$_SERVER["PHP_SELF"])) > 0) && isset($_POST)))     {
  ...
  @session_write_close();
  set_time_limit(0);
  $EmailRef = "waue_mscr_Reviewer2ConfirmNo_1";
  $BurstSize = 200;
  $BurstTime = 1;
  $WaitTime = 1;
  ...
  while ($RecipIndex < sizeof($RecipArray))  {
    ...
    while (($EnteredValue && $CurIndex < sizeof($RecipArray[$RecipIndex])) || (!$EnteredValue && $RecipArray[$RecipIndex][0])) {
      ...
      wa_sleep($WaitTime);
      include("../webassist/email/waue_mscr_Reviewer2ConfirmNo_1.php");
