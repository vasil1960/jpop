...
?>
<?php
$colname_rsEditorsLeter = "163";
if (isset($_GET['mnscrpt_id'])) {
  $colname_rsEditorsLeter = $_GET['mnscrpt_id'];
}
mysql_select_db($database_localhost, $localhost);
$query_rsEditorsLeter = sprintf("SELECT mnscrpts.mscrID, mnscrpts.mscrAutorID, mnscrpts.mscrFullTitle, mnscrpts.mscrCodeU, mnscrpts.mscrEditorsLetter, users.UserEmail, CONCAT(users.UserFirstName,' ', users.UserLastName) AS FullAutorName, mnscrpts.mscrUpldDate, mnscrpts.mscrDateEditorsLetter FROM mnscrpts, users WHERE mnscrpts.mscrAutorID=users.UserID AND mnscrpts.mscrID=%s", GetSQLValueString($colname_rsEditorsLeter, "int"));
    ...
    <div id="mainContent">
      <div id="form1_ProgressWrapper">
        <form action="" method="post" enctype="multipart/form-data" name="form1" id="form1">
          <span class="title">Editor's Decision Comment</span>
          <hr />
          <input name="dateNow" type="hidden" id="dateNow" value="<?php echo $datumZ; ?>" />
          <br />
          <table width="100%" border="0" cellpadding="0" cellspacing="0">
            <tr>
              <td width="13%" rowspan="2" align="center" bgcolor="#FEED77"><?php echo $row_rsEditorsLeter['mscrCodeU']; ?></td>
              <td width="68%" rowspan="2" bgcolor="#CCCCCC"><?php echo $row_rsEditorsLeter['mscrFullTitle']; ?></td>
              <td width="19%" align="center" bgcolor="#CCCCCC"><?php echo $row_rsEditorsLeter['FullAutorName']; ?></td>
            </tr>
            <tr>
              <td align="center" bgcolor="#CCCCCC"><?php echo $row_rsEditorsLeter['mscrUpldDate']; ?></td>
            </tr>
            <tr>
              <td>&nbsp;</td>
              <td width="68%">&nbsp;</td>
            ...
            <tr>
              <td>&nbsp;</td>
              <td><input name="txtEditorsLetter" type="file" id="txtEditorsLetter" size="40" /></td>
              <td>&nbsp;</td>
            </tr>
            <tr>
              <td>&nbsp;</td>
              <td class="validation_text"><?php
if (ValidatedField('mscrUpldEditorsLeter_734','mscrUpldEditorsLeter_734'))  {
  if ((strpos((",".ValidatedField("mscrUpldEditorsLeter_734","mscrUpldEditorsLeter_734").","), "," . "1" . ",") !== false || "1" == ""))  {
    if (!(false))  {
?>
                Please select the editor's letter to the author.
                <?php //WAFV_Conditional mscr_UpldEditorsLeter.php mscrUpldEditorsLeter_734(1:)
    }
  }
}
?></td>
            ...
            <tr>
              <td>&nbsp;</td>
              <td><input type="submit" name="button" id="button" value="Upload Editor's Letter" />
...
<!-- InstanceEnd --></html>
<?php
mysql_free_result($rsEditorsLeter);
?>
