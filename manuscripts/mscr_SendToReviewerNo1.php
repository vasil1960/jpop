...
$rsMnscrpt->setQuery("SELECT mnscrpts.mscrID,  mnscrpts.mscrFullTitle, mnscrpts.mscrCodeU, mnscrpts.mscrUpldDate, CONCAT(users.UserFirstName,' ', users.UserLastName) AS  AutorFullName, mnscrpts.mscrReviewerFullName_1, mnscrpts.mscrReviewerFullName_2, mnscrpts.mscrReviewerFullName_3, mnscrpts.mscrAutorID, mnscrpts.mscrReviewer_1, mnscrpts.mscrReviewer_2, mnscrpts.mscrReviewer_3, users.UserEmail, mnscrpts.mscrFileName FROM mnscrpts, users WHERE mnscrpts.mscrID=? AND users.UserID=mnscrpts.mscrAutorID");
$rsMnscrpt->bindParam("i", "".($_GET['mnscrpt_id'])  ."", "1"); //colname
$rsMnscrpt->execute();
?>
<?php
$rsReviewer_1 = new WA_MySQLi_RS("rsReviewer_1",$localhost_i,1);
$rsReviewer_1->setQuery("SELECT users.UserID,  CONCAT(users.UserFirstName,' ', users.UserLastName,'   ','( ',users.UserEmail ,' ) ') AS FullName, users.UserLavel, status.statusID, users.UserEmail, CONCAT(users.UserFirstName,' ', users.UserLastName) AS ReviewerFullName FROM users, status WHERE status.statusID= users.UserLavel AND status.statusID>=2 AND status.statusID<=3 AND users.UserID=?");
$rsReviewer_1->bindParam("i", "".((isset($_POST["txtReviewer_1"]))?$_POST["txtReviewer_1"]:"")  ."", "-1"); //colname
$rsReviewer_1->execute();
?>
<?php
...
if ("" === "") {
  ...
  $InsertQuery->bindColumn("logContent", "s", "Send To Reviewer No 1", "WA_BLANK");
...
if (($_SERVER["REQUEST_METHOD"] === "POST") && (isset($_SERVER["HTTP_REFERER"]) && strpos(urldecode($_SERVER["HTTP_REFERER"]), urldecode($_SERVER["SERVER_NAME"].$_SERVER["PHP_SELF"])) > 0) && isset($_POST)) {
  ...
  $UpdateQuery->bindColumn("mscrDateToReviewer_1", "s", "to reviewer: ".$datumZ  ." ", "WA_BLANK");
  $UpdateQuery->bindColumn("mnscConfirmReviewer_1", "s", "".$null  ."", "WA_BLANK");
  $UpdateQuery->bindColumn("mnscrCodeConfirmYes_1", "s", "".$_SESSION['confirmYes']  ."", "WA_BLANK");
  $UpdateQuery->bindColumn("mnscrCodeConfirmNo_1", "s", "".$_SESSION['confirmNo']  ."", "WA_BLANK");
  $UpdateQuery->bindColumn("mnscrDateConfirm_1", "s", "".$null  ."", "WA_BLANK");
  $UpdateQuery->bindColumn("mscrReviewer_1", "s", "".((isset($_POST["txtReviewer_1"]))?$_POST["txtReviewer_1"]:"")  ."", "WA_BLANK");
  $UpdateQuery->bindColumn("mscrReviewerFullName_1", "s", "".$rsReviewer_1->getColumnVal('ReviewerFullName')  ."", "WA_BLANK");
...
if ($_SERVER["REQUEST_METHOD"] === "POST")     {  //WA Universal Email
  $Email = new WA_Email("waue_mscr_SendToReviewer_1");
  ...
  $Email->WaitTime = "1";
  $Email->CharSet = "UTF-8";
  $Email->addTo("".$rsReviewer_1->getColumnVal('UserEmail')  ."");
  $Email->addTo("ivilievltu@yahoo.com");
  $Email->addTo("v.tsigov@yahoo.com");
  $Email->addTo("v.tsigov@gmail.com");
  $Email->BodyFile = "../webassist/email/waue_mscr_SendToReviewer_1_body_2.php";
          ...
          <table width="100%" border="0" cellpadding="0" cellspacing="2">
            ...
            <tr>
              ...
              <td colspan="3"><select name="txtReviewer_1" id="txtReviewer_1">
                <option value="" <?php if (!(strcmp("", $rsMnscrpt->getColumnVal('mscrReviewer_1')))) {echo "selected=\"selected\"";} ?>>Select reviewer ...</option>
                <?php
while(!$rsReviewer->atEnd()) { //dyn select
?>
                <option value="<?php echo $rsReviewer->getColumnVal('UserID')?>"<?php if (!(strcmp($rsReviewer->getColumnVal('UserID'), $rsMnscrpt->getColumnVal('mscrReviewer_1')))) {echo "selected=\"selected\"";} ?>><?php echo $rsReviewer->getColumnVal('FullName')?></option>
                <?php
  $rsReviewer->moveNext();
} //dyn select
...
?>
              ...
              <td width="1%">&nbsp;</td>
              <td width="1%">&nbsp;</td>
              <td colspan="2"><a href="mscr_Reminder_1.php?mnscrpt_id=<?php echo $rsMnscrpt->getColumnVal('mscrID'); ?>">Reminder</a></td>
            </tr>
            <tr>
              <td><strong>Reviewer No 2:</strong></td>
              <td colspan="3"><?php echo $rsMnscrpt->getColumnVal('mscrReviewerFullName_2'); ?></td>
              <td>&nbsp;</td>
              <td>&nbsp;</td>
              <td colspan="2">&nbsp;</td>
            </tr>
