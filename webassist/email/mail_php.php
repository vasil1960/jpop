	...
	if ($recEmail != "")     {
		...
		for ($rec=0; $rec<sizeof($recArray); $rec++)     {
			if (WAUE_isEmailAddress($recArray[$rec][1])) {
				$mailObj->recipients[] = array($recArray[$rec][1], $recArray[$rec][0], $recArray[$rec][1]);
			}
			else {
				$mailObj->recipients[] = array("", $recArray[$rec][0], $recArray[$rec][1]);
			}
		}
	}
	return $mailObj;
...
function WAUE_SendMail($mailObj,$mailAttachments,$mailBCC,$mailCC,$mailTo,$mailImportance,$mailFrom,$mailSubject,$mailBody,$mailRef="WAUE")     {
	...
	if ($mailTo != "") {
		...
		$_SESSION[$mailRef."_To"] = ( ($failedTo != "") ? $failedTo : $mailTo );
		$_SESSION[$mailRef."_Subject"] = $mailSubject;
		$_SESSION[$mailRef."_Body"] = "*NOT SENT, INVALID TO ADDRESS*";
		$_SESSION[$mailRef."_Header"] = "*NOT SENT, INVALID TO ADDRESS*";
		$_SESSION[$mailRef."_Log"] .=  "Sending To: " . ( ($failedTo != "") ? $failedTo : $mailTo ) . "... ";
		$_SESSION[$mailRef."_Error"] = "Invalid email address.";
		$_SESSION[$mailRef."_Status"] = "Failure";
		$_SESSION[$mailRef."_Log"] .=  ( (isset($_SESSION[$mailRef."_Error"])) ? $_SESSION[$mailRef."_Error"] : "" ). " - Failure <br>\n";
		$mailObj = false;
	}
