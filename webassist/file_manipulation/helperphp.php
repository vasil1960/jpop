...
function WA_DFP_DownloadFile($statusName, $folderPath, $fileName, $newFileName, $updateDB, $dbName, $connectionName, $tableName, $keyColumn, $recordID, $countColumn){
	...
	if (strpos($fileName, "/") !== false) {
		$folderPath .= $separator . substr($fileName, 0, strrpos($fileName, "/") + 1);
	...
	if(file_exists($path)){
      ...
      if (is_object($connectionName) && get_class($connectionName) == "mysqli") {
	      mysqli_query($connectionName, $query_cmd) or die(mysqli_error($connectionName));
	      $isMySQLi = true;
	    } else {
...
function WA_DFP_ResizeImage($imageFilePath, $options){
	...
	switch($options['ResizeType']){
      ...
      imagecopyresampled($tmp,$src,round(($maxWidth-$newWidth)/2),round(($maxHeight-$newHeight)/2),0,0,$newWidth,$newHeight,$width,$height);
			$newWidth = $maxWidth;
			$newHeight = $maxHeight;
			break;
